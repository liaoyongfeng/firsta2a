...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(`${apiUrl}/api/secondme/user/softmemory`, {
      headers: {
    ...
    console.error('Get user soft memory error:', error);
    return NextResponse.json(
      { error: '获取软记忆失败', details: error instanceof Error ? error.message : '未知错误' },
      { status: 500 }
    );
  }
}
